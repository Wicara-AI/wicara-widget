...
const sortMessagesByDate = (messages: (InboundMessage  & {timestamp: Date})[]) => {
...
export const useMessages = ({
  ...
  const [messages, setMessages] = useState<(InboundMessage & {timestamp: Date})[]>([]);
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (newMessage: NewMessageProps) => {
      ...
      setMessages(prevMessages => {
        ...
        const messageExists = prevMessages.some(msg => msg.id === newMessage.message.id);
