...
import { sendMessage as apiSendMessage, UploadFileRequest } from '../utilities/api';
...
export const useSendMessage = ({apiHeaders}: UseSendMessage) => {
  ...
  const createMessageBuilder = (options: SendMessageOptions, replyId?: string) => {
  ...
  const sendSingleMessage = async (
    ...
    await apiSendMessage(outboundMessage, apiHeaders as any, abortController.signal);
  };
