...
export function BubbleChat({
  ...
  const [activeScreen, setActiveScreen] = useState<string>(
    screenConfig.find((item) => item.root)!.id,
  )
  ...
  useEffect(() => {
    ...
    if (
      storageLastScreen !== null &&
      storageLastScreen !== undefined &&
      storageLastScreen !== activeScreen
    ) {
      ...
      >
        <AuthProvider>
          ...
          <BubbleDialog ref={dialogChatRef} open={isOpenDialogChat}>
            ...
            <BubbleDialogContent>
              {screenConfig
                .filter((item) => item.id === activeScreen)
                .map(({ screen: Screen, mustAuth }, key: number) => (
                  <div key={key}>
                    {mustAuth ? (
